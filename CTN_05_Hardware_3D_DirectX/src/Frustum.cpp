...
Frustum::Frustum(Graphics& gfx, float width, float height, float nearZ, float farZ)
{
...

	{
		Technique line{ Chan::main };
		{
			...
			auto pvs = VertexShader::Resolve(gfx, "ShaderBins\\Solid_VS.cso");
			unoccluded.AddBindable(InputLayout::Resolve(gfx, pVertices->GetLayout(), *pvs));
			unoccluded.AddBindable(std::move(pvs));

			unoccluded.AddBindable(PixelShader::Resolve(gfx, "ShaderBins\\Solid_PS.cso"));
		...
		}
		{
			...
			occluded.AddBindable(PixelShader::Resolve(gfx, "ShaderBins\\Solid_PS.cso"));
